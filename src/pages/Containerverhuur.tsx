...
import { Package, CheckCircle2, Truck, ArrowRight, Phone, AlertTriangle, MapPin, Clock, FileText, Construction, Recycle, TreePine } from "lucide-react";
...
const wasteCategories = [
  {
    ...
    items: [
      { name: "Steenpuin" },
      { name: "Aarde", note: "Met en zonder stenen" },
...
const ContainerverhuurPage = () => {
        ...
        <div className="container mx-auto px-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8 max-w-6xl mx-auto">
            {wasteCategories.map((cat, idx) => (
              <FadeInView key={idx} delay={idx * 0.1} className="bg-card/50 border border-border p-6 rounded-sm">
                <div className="flex items-center gap-3 mb-6 border-b border-primary/20 pb-4">
                  {cat.icon}
                  <h3 className="font-display text-lg text-primary uppercase tracking-tight">{cat.category}</h3>
                </div>
                <ul className="space-y-4">
                  {cat.items.map((item, i) => (
                    <li key={i} className="flex items-start gap-3">
                      ...
                      <div>
                        <span className="text-card-foreground text-sm font-bold uppercase block leading-tight">{item.name}</span>
          ...
          <FadeInView className="mt-12 max-w-3xl mx-auto" delay={0.3}>
            <div className="bg-card border border-primary/30 rounded-sm p-4 md:p-6 flex items-start gap-4">
              ...
              <div>
                <h4 className="font-display text-base md:text-lg text-primary uppercase tracking-wider mb-2">Belangrijke informatie</h4>
                <p className="text-muted-foreground text-xs md:text-sm leading-relaxed">
                  Sommige afvalstromen zoals asbest of asfalt met koolteer vereisen specifieke procedures. Neem bij twijfel of voor grote projecten altijd contact met ons op voor de meest recente richtlijnen.
                </p>
              </div>
            </div>
          </FadeInView>
  ...
  Wij beschikken over een eigen <strong className="text-card-foreground font-bold">breekinstallatie</strong> en <strong className="text-card-foreground font-bold">zeefinstallatie</strong>. Hierdoor hebben wij diverse recuperatiematerialen en opvoermateriaal direct ter beschikking voor uw project. Recuperatiemateriaal kan worden afgehaald of geleverd.
</p>
              <ul className="space-y-3 mb-8">
                <li className="flex items-center gap-3 text-sm md:text-base">
                  <CheckCircle2 className="h-5 w-5 text-primary flex-shrink-0" />
                  <span className="text-card-foreground font-semibold">Constante voorraad van gekeurde materialen</span>
                </li>
                <li className="flex items-center gap-3 text-sm md:text-base">
                  <CheckCircle2 className="h-5 w-5 text-primary flex-shrink-0" />
                  <span className="text-card-foreground font-semibold">Alles in eigen beheer voor scherpe prijzen</span>
                </li>
              </ul>
            </FadeInView>

            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <FadeInView delay={0.2} className="relative group overflow-hidden border border-border bg-card aspect-square">
                <img src="/placeholder.svg" alt="Breekinstallatie" className="w-full h-full object-cover grayscale group-hover:grayscale-0 transition-all duration-700" />
                <span className="absolute bottom-4 left-4 font-display text-primary uppercase text-[10px] md:text-xs z-10">Breekinstallatie</span>
              </FadeInView>
              <FadeInView delay={0.3} className="relative group overflow-hidden border border-border bg-card aspect-square">
                <img src="/placeholder.svg" alt="Zeefinstallatie" className="w-full h-full object-cover grayscale group-hover:grayscale-0 transition-all duration-700" />
                <span className="absolute bottom-4 left-4 font-display text-primary uppercase text-[10px] md:text-xs z-10">Zeefinstallatie</span>
              </FadeInView>
            </div>
          </div>

          {/* Recuperatiemateriaal Opties */}
          <FadeInView className="bg-card/50 border border-border p-6 md:p-10 rounded-sm">
            <h3 className="font-display text-xl md:text-2xl text-primary uppercase mb-8 border-b border-primary/20 pb-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              ...
              <div className="flex gap-4 items-start">
                ...
                <div>
                  <h4 className="text-card-foreground font-bold uppercase tracking-tight mb-1">Gebroken Mengpuin 0/56</h4>
                  <p className="text-xs text-muted-foreground leading-snug">
                    Met keuringsattest (certificaatnummer CRT/LB001/Q42, gerecycleerde granulaten vaste locatie met breker)
                  </p>
                </div>
              </div>
              ...
              <div className="flex gap-4 items-start">
                ...
                <div>
                  <h4 className="text-card-foreground font-bold uppercase tracking-tight mb-1">Zeefzand</h4>
                  <p className="text-xs text-muted-foreground">Hoogwaardig zand voor diverse toepassingen.</p>
                </div>
              </div>
              ...
              <div className="flex gap-4 items-start">
                ...
                <div>
                  <h4 className="text-card-foreground font-bold uppercase tracking-tight mb-1">Zeefgrond</h4>
                  <p className="text-xs text-muted-foreground">Gezuiverde grond, ideaal voor ophogingen.</p>
                </div>
              </div>
              ...
              <div className="flex gap-4 items-start">
                ...
                <div>
                  <h4 className="text-card-foreground font-bold uppercase tracking-tight mb-1">Teelaarde</h4>
                  <p className="text-xs text-muted-foreground">Kwalitatieve grond voor tuin en landschap.</p>
                </div>
              </div>
            </div>
          </FadeInView>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 md:gap-12 items-stretch">
            <FadeInView direction="left" className="h-full">
              <div className="group relative h-full bg-card/50 border border-border p-6 md:p-10 hover:border-primary transition-all duration-500 overflow-hidden flex flex-col">
                <span className="absolute -right-4 -bottom-8 text-7xl md:text-[12rem] font-display font-black text-primary/5 group-hover:text-primary/10 transition-colors pointer-events-none">01</span>
                <div className="relative z-10 flex-grow">
                  <div className="flex items-center gap-4 md:gap-5 mb-6 md:mb-8">
                    ...
                    <h3 className="font-display text-2xl md:text-3xl text-primary uppercase">Zelf Aanvoeren</h3>
                  </div>
                  <p className="text-lg md:text-xl text-card-foreground font-semibold mb-6">Beschikt u over eigen transport? Breng uw afvalstoffen direct naar ons terrein.</p>
                ...
                <Button variant="hero" size="xl" className="w-full relative z-10 mt-auto" asChild>
                  <a href="tel:0476926625"><Phone className="h-5 w-5 mr-2" /> Bel voor afspraak</a>
                </Button>
              </div>
            </FadeInView>
            ...
            <FadeInView direction="right" delay={0.2} className="h-full">
              <div className="group relative h-full bg-card/50 border border-border p-6 md:p-10 hover:border-primary transition-all duration-500 overflow-hidden flex flex-col">
                <span className="absolute -right-4 -bottom-8 text-7xl md:text-[12rem] font-display font-black text-primary/5 group-hover:text-primary/10 transition-colors pointer-events-none">02</span>
                <div className="relative z-10 flex-grow">
                  <div className="flex items-center gap-4 md:gap-5 mb-6 md:mb-8">
                    <div className="w-12 h-12 md:w-16 md:h-16 bg-primary flex items-center justify-center shadow-[0_0_20px_rgba(255,255,0,0.3)]">
                      <Truck className="h-6 w-6 md:h-8 md:w-8 text-background" />
                    </div>
                    <h3 className="font-display text-2xl md:text-3xl text-primary uppercase">Afhalen of Leveren</h3>
                  </div>
                  <p className="text-lg md:text-xl text-card-foreground font-semibold mb-6 leading-tight">Flexibele logistiek voor al uw recuperatie- en opvoermateriaal.</p>
                  <div className="grid grid-cols-2 gap-3 md:gap-4 mb-10">
                    <div className="p-3 md:p-6 border border-border bg-background/50 text-center">
                      <span className="block font-display text-base md:text-2xl text-primary uppercase">Afhalen</span>
                    </div>
                    <div className="p-3 md:p-6 border border-border bg-background/50 text-center">
                      <span className="block font-display text-base md:text-2xl text-primary uppercase">Leveren</span>
                    </div>
                  </div>
                </div>
                <Button variant="hero" size="xl" className="w-full relative z-10 mt-auto" asChild>
                  <a href="tel:0476926625"><Package className="h-5 w-5 mr-2" /> Bestel Materiaal</a>
                </Button>
              </div>
            </FadeInView>
          </div>
      ...
      <section className="py-16 md:py-20 bg-card border-t border-border text-center">
        <div className="container mx-auto px-4">
          <FadeInView className="max-w-3xl mx-auto">
            <h2 className="font-display text-2xl md:text-4xl text-primary uppercase tracking-wider mb-6">Container Nodig?</h2>
            <p className="text-sm md:text-lg text-muted-foreground mb-8">Vraag vandaag nog een offerte aan. Wij zorgen voor snelle levering en ophaling.</p>
